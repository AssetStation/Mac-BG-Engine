...
def remove_bg(input_path, output_path, model_name="u2net.onnx", use_gpu=False, edge_shift=0, edge_feather=2):
    try:
        ...
        # 1. Binarize at a lower threshold (32 instead of 128) to preserve more of the object's true edges
        _, mask = cv2.threshold(mask, 32, 255, cv2.THRESH_BINARY)
        
        # 2. Edge Shift (Dilate or Erode)
        if edge_shift > 0:
            kernel = np.ones((edge_shift*2+1, edge_shift*2+1), np.uint8)
            mask = cv2.dilate(mask, kernel, iterations=1)
        elif edge_shift < 0:
            val = abs(edge_shift)
            kernel = np.ones((val*2+1, val*2+1), np.uint8)
            mask = cv2.erode(mask, kernel, iterations=1)
            
        # 3. Edge Feather (Gaussian Blur)
        if edge_feather > 0:
            k_size = edge_feather * 2 + 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--edge-shift", type=int, default=0, help="Erode (<0) or Dilate (>0) mask")
    parser.add_argument("--edge-feather", type=int, default=2, help="Gaussian blur feather amount")
    
    args = parser.parse_args()
    
    remove_bg(args.input, args.output, args.model, args.gpu, args.edge_shift, args.edge_feather)